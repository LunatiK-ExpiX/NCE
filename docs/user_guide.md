...
To download a precompiled version of NCE, locate the "tags" button in the [NCE repository](https://github.com/Nivaturimika/Katerina-Engine).
...
Then simply click on the `NCE_Launcher` executable.

- `NCE`: The main game executable, do not open it directly.
- `NCE_Launcher`: The game launcher, you need to create/recreate scenarios for each change you do to the mods.
...
- `assets`: Folder NEEDED for NCE to work properly.

## Ubuntu/PopOS

To install the game on Ubuntu/PopOS, follow the same steps for installation and download as for Windows until the point of launching `NCE_Launcher`. At that point, download [Lutris](https://lutris.net).
...
- Check that your firewall is not blocking NCE
